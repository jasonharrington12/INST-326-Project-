...
The system uses Python's standard libraries (`json`, `csv`, `pathlib`) for robust data handling and persistence.
...
1.  **Ensure Files are Present:** Make sure all files (`main_cli.py`, `library_model.py`, `persistence_manager.py`, `test_library.py`) and the required `data/` folder (which will be created automatically) are present in your project directory.
2.  **Run the Main Script:** Execute the main program from your terminal:

    ```bash
    python main_cli.py
    ```
3.  **Initial Load:** The program will first attempt to load the state from `data/library_state.json`. If it's a first run, it will automatically create the `data/` folder and start with an empty catalog (unless your `main_cli.py` is configured to load default items).
...
* `test_import.csv`: Created by the test suite to verify the required data import functionality.
...
This project successfully implements the following key technical requirements:

### Object-Oriented Programming (OOP)

* **Inheritance:** `Book`, `DVD`, and `EBook` classes inherit from the abstract base class `LibraryItem`.
* **Polymorphism:** Each item type implements its own `calculate_loan_period()`, resulting in different checkout durations (e.g., Book = 14 days, DVD = 3 days).
...
* **Import Feature:** The `import_items_from_csv` method (correctly named in `persistence_manager.py`) reads and integrates new data from a standard CSV file.
* **Export Feature:** The `export_loan_report` method generates a readable text report (`.txt`) of current loans.

### Testing and Best Practices

* **Comprehensive Testing:** `test_library.py` includes **4 Unit Tests**, **5 Integration Tests**, and **3 System Tests** (including the full Save/Load cycle and CSV Import/Report Export workflows).
